//  The multiavatar library with a few customizations like themeOverride and BlackandWhite paramters.
...
import 'dart:ui' as ui;
  ...
  String multiavatar(String string, {bool trBackground = false, String? themeOverride, bool makeBW= false }) {
  string += '';
  ...
  String? convertColorToGrayscale(String color) {
    if (color.length == 7 && color.startsWith('#')) {
      int colorValue = int.parse(color.substring(1), radix: 16) + 0xFF000000;
      ui.Color c = ui.Color(colorValue);

      int grayValue = ((c.red * 0.299) + (c.green * 0.587) + (c.blue * 0.114)).round();
      ui.Color grayColor = ui.Color.fromARGB(255, grayValue, grayValue, grayValue);

      ui.Color halfwayColor = ui.Color.lerp(c, grayColor, 0.9)!;
      return '#${halfwayColor.value.toRadixString(16).substring(2).padLeft(6, '0')}';
    } else {
      return color;
    }
  }
  ...
  String getFinal(part, partV, theme) {
    ...
    List<String?>? colors2 = themes[partV]?[theme]?[part];
    List<String?> colors =[];

    if (colors2 != null) {
      for (int i = 0; i < colors2.length; i++) {
        String? my_color_item = colors2[i];
        if (my_color_item != "none" && my_color_item != null) {
          if (makeBW) {
            my_color_item = convertColorToGrayscale(my_color_item);
          }
        }

        colors.add(my_color_item);
      }
    }
  ...
  for (var part in p.keys) {
    ...
    //themeOverride = null;
    if (themeOverride != null && themeOverride != "" && themeOverride != "default") {
      partV = themeOverride.substring(0, 2);
    }
